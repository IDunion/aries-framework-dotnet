...
using Xunit;
using static Hyperledger.Aries.Tests.Protocols.ConnectionTestsV2;

namespace Hyperledger.Aries.Tests.Integration
{
    [Trait("Category", "DefaultV1")]
    public class DidExchangeTestsV1 : IAsyncLifetime
    {
        static DidExchangeTestsV1()
        {
    ...
    {
        private WalletConfiguration _config1 = new WalletConfiguration { Id = Guid.NewGuid().ToString() };
        private WalletConfiguration _config2 = new WalletConfiguration { Id = Guid.NewGuid().ToString() };
        private WalletCredentials _cred = new WalletCredentials { Key = "2" };
        ...
        public async Task InitializeAsync()
        {
            _responder = await MockUtils.CreateAsync("responder", _config1, _cred, new MockAgentHttpHandler((cb) => _router.RouteMessage(cb.name, cb.data)), TestConstants.StewardSeed);
            _router.RegisterAgent(_responder);
            _requester = await MockUtils.CreateAsync("requester", _config2, _cred, new MockAgentHttpHandler((cb) => _router.RouteMessage(cb.name, cb.data)));
            _router.RegisterAgent(_requester);
