...
namespace Hyperledger.Aries.Features.PresentProof
{
    public class DefaultProofServiceV2 : IProofService
    {
            ...
            ProofRequest proofRequest, RequestedCredentials requestedCredentials)
        {
            ...
            //There can be more credProofJsons relating to one entry in credEntryJson indicated by index in credProofJsons
            //index 0 relates to first element of credentialEntryJsons, index 1 to second and so on
            var credentialEntryProofMapping = new Dictionary<string, int>();
            int index = 0;
            ...
            foreach (var attr in requestedCredentials.RequestedAttributes)
            {
                ...
                //check if credential already added to credentialObjects, no need for adding same credential in credentialEntryJsons and building revocationStates several times
                if (!credentialObjects.Any(x => x.Referent.Equals(credentialRecord.Id)))
                {
                    credentialEntryProofMapping.Add(credentialRecord.Id, index);
                    ...
                    if (credentialRecord.RevocationRegistryId != null)
                    {
                        ...
                        Debug.WriteLine($"Hyperledger Aries after CreateOrUpdateRevocationStateAsync() - revocationStateJson: {revStateJson}");
                ...
                {
                    credentialEntryProofMapping.TryGetValue(credentialRecord.Id, out int idx);
                    Debug.WriteLine($"Hyperledger Aries - Credential referent already in list credentialObjects: '{credentialRecord.Id}' relating to index '{idx}'");
                }

                //build credentialProofJson list: relate prove to a credential which was found for a given attr.Key by setting right index from credentialEntryProofMapping
                credentialEntryProofMapping.TryGetValue(credentialRecord.Id, out int proofIndex);
                credentialProofJsons.Add(JsonConvert.SerializeObject(new CredentialProof
                {
                    EntryIndex = proofIndex,
            ...
            {
                Debug.WriteLine($"Hyperledger Aries - Processing attribute: '{JsonConvert.SerializeObject(pred)}'");
                ...
                //build credentialProofJson list: relate prove to a credential which was found for a given pred.Key by setting right index from credentialEntryProofMapping
                credentialEntryProofMapping.TryGetValue(credentialRecord.Id, out int proofIndex);
                credentialProofJsons.Add(JsonConvert.SerializeObject(new CredentialProof
                {
                    EntryIndex = proofIndex,
